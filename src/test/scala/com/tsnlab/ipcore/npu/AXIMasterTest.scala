...
class AXIMasterTest extends AnyFreeSpec with ChiselScalatestTester with ParallelTestExecution {
  ...
  "AXI Master - Simulate and test bus read" in {
    test(new M_AXI(axiparam)).withAnnotations(Seq(WriteVcdAnnotation)) {
      aximaster => {
        ...
        aximaster.memport_r.burstlen.poke(0.U)
        aximaster.memport_w.data.poke("hDEAD_F00D".U)
        aximaster.memport_w.burstlen.poke(0.U)
  ...
  "AXI Master - Simulate and test burst bus read" in {
    test(new M_AXI(axiparam)).withAnnotations(Seq(WriteVcdAnnotation)) {
      aximaster => {
        var busslaveState: BusState = ARREADY
        ...
        // Some default values
        aximaster.memport_r.addr.poke("hCA7F_00D0".U)
        aximaster.memport_r.burstlen.poke(7.U)
        aximaster.memport_w.data.poke("hDEAD_F00D".U)
        aximaster.memport_w.burstlen.poke(0.U)
  ...
  "AXI Master - Simulate and test burst bus write" in {
    test(new M_AXI(axiparam)).withAnnotations(Seq(WriteVcdAnnotation)) {
      aximaster => {
        var busslaveState: BusState = AWREADY
        // Some default values
        aximaster.memport_w.data.poke("hDEAD_F00D".U)
        aximaster.memport_w.addr.poke("hCA7F_00D0".U)
        aximaster.memport_w.burstlen.poke(7.U)
        ...
        for (i <- 1 to 512) {
          ...
          busslaveState match {
            ...
            case WREADY => {
              ...
              if (aximaster.M_AXI.wvalid.peek().litValue == 1) {
                aximaster.M_AXI.wdata.expect("hDEAD_F00D".U)
                if (aximaster.M_AXI.wlast.peek().litValue == 1) {
                  busslaveState = BREADY
                } else {
                  busslaveState = WREADY
                }
